...
def parse_args():
    ...
    if any(challenge_id not in IMPLEMENTED_CHALLENGES for challenge_id in args.challenge):
        raise ValueError(f"Invalid or unimplemented challenge id. Recognized challenge ids are: {str(IMPLEMENTED_CHALLENGES)}")
    return args
...
def copy_binaries_to_maelstrom_solutions(challenges: typing.List[int]):
    ...
    for file_path in os.listdir(SRC_DIR):
        ...
        if os.path.isfile(src_file_path) and os.access(src_file_path, os.X_OK):
            if not any(file_path.startswith(f"{challenge_id:02d}") for challenge_id in challenges):
                continue
...
def run_test(challenge: int, time_limit: int):
    ...
    logger.info("Running maelstrom test workload %s for challenge \"%s\" (id: %s)", CHALLENGE_WORKLOADS[challenge], IMPLEMENTED_CHALLENGES[challenge], challenge)
